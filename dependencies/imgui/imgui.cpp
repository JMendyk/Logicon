...
bool ImGui::OpenPopupOnItemClick(const char* str_id, int mouse_button)
{
    ...
    if (IsMouseReleased(mouse_button) && IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup)) {
        ...
        IM_ASSERT(id !=
                  0);                                                  // However, you cannot pass a NULL str_id if the last item has no identifier (e.g. a Text() item)
        OpenPopupEx(id);
        return true;
    }
    return false;
...
static ImVec2
FindBestWindowPosForPopup(const ImVec2 &ref_pos, const ImVec2 &size, ImGuiDir *last_dir, const ImRect &r_avoid,
...
// Combo Box policy (we want a connecting edge)
    if (policy == ImGuiPopupPositionPolicy_ComboBox) {
        const ImGuiDir dir_prefered_order[ImGuiDir_Count_] = {ImGuiDir_Down, ImGuiDir_Right, ImGuiDir_Left,
                                                              ImGuiDir_Up};
...
// When the window cannot fit all contents (either because of constraints, either because screen is too small): we are growing the size on the other axis to compensate for expected scrollbar. FIXME: Might turn bigger than DisplaySize-WindowPadding.
        ...
        if (size_auto_fit_after_constraint.x < size_contents.x && !(flags & ImGuiWindowFlags_NoScrollbar) &&
...
// This is how we end up with child menus appearing (most-commonly) on the right of the parent menu.
            ...
            if (parent_menu->DC.MenuBarAppending)
                ...
                rect_to_avoid = ImRect(parent_menu->Pos.x + horizontal_overlap, -FLT_MAX,
                                       parent_menu->Pos.x + parent_menu->Size.x - horizontal_overlap -
                                       parent_menu->ScrollbarSizes.x, FLT_MAX);
...
// Position tooltip (always follows mouse)
        if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api) {
            ...
            ImRect rect_to_avoid(ref_pos.x - 16, ref_pos.y - 8, ref_pos.x + 24, ref_pos.y +
                                                                                24); // FIXME: Completely hard-coded. Store boxes in mouse cursor data? Scale? Center on cursor hit-point?
            window->PosFloat = FindBestWindowPosForPopup(ref_pos, window->Size, &window->AutoPosLastDirection,
                                                         rect_to_avoid);
            if (window->AutoPosLastDirection == ImGuiDir_None)
                window->PosFloat = ref_pos + ImVec2(2,
...
// Title bar only
            ...
            RenderFrame(title_bar_rect.Min, title_bar_rect.Max, GetColorU32(ImGuiCol_TitleBgCollapsed), true,
                        window_rounding);
...
// Handle resize for: Resize Grips, Borders, Gamepad
            ...
            const float grip_draw_size = (float) (int) ImMax(g.FontSize * 1.35f,
                                                             window_rounding + 1.0f + g.FontSize * 0.2f);
...
// Title bar
            const bool window_is_focused =
                    want_focus || (g.NavWindow && window->RootNonPopupWindow == g.NavWindow->RootNonPopupWindow);
            if (!(flags & ImGuiWindowFlags_NoTitleBar))
                window->DrawList->AddRectFilled(title_bar_rect.Min, title_bar_rect.Max, GetColorU32(
                        window_is_focused ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg), window_rounding,
...
// Force round operator last to ensure that e.g. (int)(max.x-min.x) in user's render code produce correct result.
    ...
    clip_rect.Min.x = ImFloor(
            0.5f + window->InnerRect.Min.x + ImMax(0.0f, ImFloor(window->WindowPadding.x * 0.5f - border_size)));
    clip_rect.Min.y = ImFloor(0.5f + window->InnerRect.Min.y);
    clip_rect.Max.x = ImFloor(
            0.5f + window->InnerRect.Max.x - ImMax(0.0f, ImFloor(window->WindowPadding.x * 0.5f - border_size)));
...
// Minor hack to to make scrolling to top/bottom of window take account of WindowPadding, it looks more right to the user this way
    if (center_y_ratio <= 0.0f && window->ScrollTarget.y <= window->WindowPadding.y)
        window->ScrollTarget.y = 0.0f;
             ...
             window->ScrollTarget.y >= window->SizeContents.y - window->WindowPadding.y + GImGui->Style.ItemSpacing.y)
...
bool ImGui::ArrowButton(ImGuiID id, ImGuiDir dir, ImVec2 padding, ImGuiButtonFlags flags) {
    ...
    const ImRect bb(window->DC.CursorPos,
                    window->DC.CursorPos + ImVec2(g.FontSize + padding.x * 2.0f, g.FontSize + padding.y * 2.0f));
    ItemSize(bb, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
...
// Unframed typed for tree nodes
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderTriangle(bb.Min + ImVec2(padding.x, g.FontSize * 0.15f + text_base_offset_y),
...
static float GetMinimumStepAtDecimalPrecision(int decimal_precision)
{
    ...
    return (decimal_precision >= 0 && decimal_precision < 10) ? min_steps[decimal_precision] : powf(10.0f,
...
// Always consume the SetNextWindowSizeConstraint() call in our early return paths
    ...
    RenderTriangle(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y),
                   ImGuiDir_Down);
...
// Menu inside a menu
    ...
    if (window->DC.LayoutType !=
        ImGuiLayoutType_Horizontal) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
...
// Drag and Drop Target
    if (window->DC.LastItemRectHoveredRect &&
        BeginDragDropTarget()) // NB: The LastItemRectHoveredRect test is merely an optional micro-optimization
    {
...
// Clamp position
        ...
        if (!(columns->Flags & ImGuiColumnsFlags_NoForceWithinWindow))
            t = ImMin(t, PixelsToOffsetNorm(columns, (columns->MaxX - columns->MinX) -
                                                     g.Style.ColumnsMinSpacing * (columns->Count - n)));
        column->OffsetNorm = t;
...
// Display individual triangles/vertices. Hover on to get the corresponding triangle highlighted.
            ...
            static void NodeWindow(ImGuiWindow *window, const char *label) {
                ...
                if (ImGui::IsItemHovered())
                    GImGui->OverlayDrawList.AddRect(window->Pos, window->Pos + window->Size,
        ...
        if (ImGui::TreeNode("Basic state")) {
            ...
            ImGui::Text("HoveredId: 0x%08X/0x%08X (%.2f sec)", g.HoveredId, g.HoveredIdPreviousFrame,
